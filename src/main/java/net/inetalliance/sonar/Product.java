...
public class Product
	...
	public static final F1<net.inetalliance.beejax.messages.Product, JsonMap> json =
			new F1<net.inetalliance.beejax.messages.Product, JsonMap>() {
				@Override
				public JsonMap $(final net.inetalliance.beejax.messages.Product product) {
					return new JsonMap()
							.$("id", product.id)
							.$("name", product.name)
							...
							.$("coupons", couponJson.map(product.coupons))
							.$("tags", Json.Factory.$(product.tags));
				}
			};
	private static final F1<Coupon, JsonMap> couponJson = new F1<Coupon, JsonMap>() {
		@Override
		public JsonMap $(final Coupon coupon) {
			return new JsonMap()
					.$("type", coupon.type)
		...
		if (matcher.matches()) {
			...
			if (StringFun.empty.$(key)) {
				...
				if (query == null) {
					...
					respond(response, new JsonMap()
							.$("products", json.map(info.products))
				...
				if (site == null) {
					final Opportunity o = Locator.$(new Opportunity(new Integer(key)));
					if (o == null) {
