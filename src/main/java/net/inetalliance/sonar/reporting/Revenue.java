...
public abstract class Revenue<R extends Named>
	...
	static Map<Integer, Query<? super Opportunity>> getCategoryQueries(final Set<Category> categories) {
	...
	protected JsonMap generate(final EnumSet<SaleSource> sources, final EnumSet<ContactType> contactTypes,
		...
		if (loggedIn == null || !(loggedIn.isManager() || loggedIn.isTeamLeader())) {
			log.warning("%s tried to access %s", loggedIn == null ? "Nobody?" : loggedIn.key, getClass().getSimpleName());
		...
		Locator.forEach(allRows(loggedIn, interval.getStart()), row -> {
			...
			if (productLine != null) {
				...
				final Query<Opportunity> notMatchingQuery =
						rowQuery.and(hasBeejaxProduct).and(Query.or(Opportunity.class, categoryQueries.values()).negate());
