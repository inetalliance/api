...
import static net.inetalliance.sonar.reporting.Revenue.CellType.DATA;
import static net.inetalliance.sonar.reporting.Revenue.CellType.NOT_MATCHING;
import static net.inetalliance.sonar.reporting.Revenue.CellType.NO_PRODUCT;
...
public class RevenueDetail
  ...
  protected void get(final HttpServletRequest request, final HttpServletResponse response)
    ...
    final SaleSource source =
        StringFun.isEmpty(mode) || "all".equals(mode) ? null
            : StringFun.camelCaseToEnum(SaleSource.class, mode);
    ...
    final Set<Category> categories =
        Arrays.stream(tagParams).map(t -> Revenue.getCategory(tagParams, Integer.valueOf(t)))
            .collect(toSet());
    final Map<Integer, Query<? super Opportunity>> categoryQueries = Revenue
    ...
    final Map<Integer, Product> productsById = products.stream()
        .collect(Funky.toMap(p -> p.id, Function.identity()));
