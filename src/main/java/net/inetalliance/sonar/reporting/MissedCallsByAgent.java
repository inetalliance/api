...
public class MissedCallsByAgent
  ...
  public MissedCallsByAgent() {
    super("site", "productLine", "callCenter");
  ...
  protected JsonMap generate(final EnumSet<SaleSource> sources,
    ...
    final Set<ProductLine> productLines = Arrays.stream(productLineIds)
      ...
      .collect(toSet());
    final Query<Call> productLinesQuery = productLines.isEmpty() ? Query.all(Call.class) :
    ...
    final Query<Call> query = Call.inInterval(interval)
      .and(Call.withOpportunity())
      .and(productLinesQuery);

    final Set<Call> calls = $$(query);
