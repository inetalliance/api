    ...
    if (o.id != null) {
      ...
      json.put("extra", new JsonMap()
        ...
        .$("assignedTo", new JsonMap()
          .$("name",
            o.getSource() == ONLINE ? "Web Order" : o.getAssignedTo().getLastNameFirstInitial()))
        .$("site", new JsonMap()
    ...
    if (support || review) {
      ...
      if (as != null && as.length > 0) {
        if (review && loggedIn.isTeamLeader()) {
          Set<String> viewableKeys =
            loggedIn.getViewableAgents().stream().map(a -> a.key).collect(toSet());
