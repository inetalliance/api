...
import static com.callgrove.obj.Opportunity.withAgentIn;
...
/**
 * this responds with filtered sales data in a json list by day that is compatible with the google
 * charts api
 */
    ...
    if (StringFun.isEmpty(cached)) {
      ...
      if (sites.isEmpty()) {
        ...
        if (!visible.containsAll(sites)) {
          ...
          log.error("%s tried to request sales data for %s, but is only granted access to %s",
              forbidden, visible);
      ...
      ProgressHandler.$
          .start(loggedIn.key, response, Weeks.weeksBetween(interval.getStart(), end).getWeeks(),
              progressMeter -> {
