...
public class Review
	...
	protected void get(final HttpServletRequest request, final HttpServletResponse response)
		...
		try {
			...
			final Query<Call> q =
					isQueue.and(isActive.negate()).and(inInterval(day.toInterval())).and(withAgentIn(viewableAgents));
			...
			forEach(q, call -> {
				...
				map.$("callerId", new JsonMap().$("name", callerId == null ? "Unknown" : callerId.getName())
				   ...
				   .$("site", site == null ? null : new JsonMap().$("abbreviation", site.getAbbreviation().toString()))
				   .$("agent",
				      agent == null ? null : new JsonMap().$("name", agent.getFirstNameLastInitial()).$("key", agent.key))
				...
				map.$("blame",
				      blame == null ? null : new JsonMap().$("name", blame.getFirstNameLastInitial()).$("key", blame.key));
