...
public abstract class Performance<R extends IdPo & Named, G extends IdPo>
  ...
  protected JsonMap generate(final EnumSet<SaleSource> sources,
    ...
    final Query<Opportunity> withMode = sources == null
      ...
      : Opportunity.Q.withSources(sources);
      ...
      public void $(final R r) {
        ...
        for (final Map.Entry<String, Interval> entry : intervals.entrySet()) {
          ...
          update(totalCalls, calls, key,
            count(callWithQueue.and(Call.Q.inInterval(interval)).and(Call.Q.notSilent)), intMath);
          update(silentCalls, silent, key,
            count(callWithQueue.and(Call.Q.inInterval(interval)).and(Call.Q.silent)), intMath);
          ...
          update(totalRevenue, revenue, key, $$(rowSales.and(
            withMode.and(Opportunity.Q.soldInInterval(interval))), SUM, Currency.class, "amount"), $Math);
        }
        meter.increment();
      }
    });
  ...
  private <N extends Number> void update(final Map<String, N> totals, final JsonMap map,
                                         final String key, final N value, final Calculator<N> calc) {
