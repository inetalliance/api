...
public abstract class Performance<R extends IdPo & Named, G extends IdPo>
	...
	protected JsonMap generate(final EnumSet<SaleSource> sources,
		...
		final Query<Opportunity> withMode = sources == null
		...
		forEach(allRows.orderBy("name", ASCENDING), r -> {
			...
			for (final Map.Entry<String, Interval> entry : intervals.entrySet()) {
				...
				update(totalCalls, calls, key,
					count(callWithQueue.and(Call.inInterval(interval)).and(Call.isNotSilent)), NumberMath.INTEGER);
				update(silentCalls, silent, key,
					count(callWithQueue.and(Call.inInterval(interval)).and(Call.isSilent)), NumberMath.INTEGER);
				...
				update(totalRevenue, revenue, key, $$(rowSales.and(
					withMode.and(Opportunity.soldInInterval(interval))), SUM, Currency.class, "amount"), Currency.MATH);
			}
			meter.increment();
		});
		...
		final JsonMap json = new JsonMap()
			...
			.$("labels",
				(JsonList) groups.stream().map(this::getGroupLabel).map(JsonString::new).collect(
