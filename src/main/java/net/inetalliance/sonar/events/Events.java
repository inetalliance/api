...
public class Events
	...
	public void onClose(final Session session, final CloseReason closeReason) {
		...
		lock.lock();
		try {
			sessions.computeIfAbsent(user.getPhone(), u -> new ArrayList<>()).remove(session);
	...
	public void onOpen(final Session session, final EndpointConfig config) {
		...
		lock.lock();
		try {
			sessions.computeIfAbsent(user.getPhone(), u -> new ArrayList<>(1))
				.add(session);
		} finally {
			lock.unlock();
		}
	...
	static void broadcast(final String type, final String agent, final Json msg) {
		...
		try {
			if (agent == null) {
				...
				sessions.computeIfAbsent(agent, a -> new ArrayList<>())
					.forEach(session -> send(session, type, msg));
			}
		} finally {
			lock.unlock();
	...
	static Set<String> getActiveAgents() {
		lock.lock();
		try {
