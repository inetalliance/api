  ...
  protected void exec()
    ...
    try (final BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(csv)))) {
      ...
      while ((line = reader.readLine()) != null) {
        cids.add(line);
        n.incrementAndGet();
        forEach(Contact.Q.withPhoneNumber(line), new P1<Contact>() {
          @Override
          public void $(final Contact contact) {
            contacts.incrementAndGet();
            forEach(Opportunity.Q.withContact(contact), new P1<Opportunity>() {
              @Override
    ...
    Locator.forEach(Call.Q.withCallerIdIn(cids), new P1<Call>() {
      @Override
    ...
    forEach(withSite($(new Site(42))).and(queue).and(inInterval(
      new Interval(new DateMidnight(2015, 7, 1), new DateMidnight(2015, 8, 1)))), new P1<Call>() {
      @Override
      public void $(final Call call) {
